...
// MARK: - Want to Watch category (movie/TV details + genres only; no extra APIs)
enum WantToWatchMediaCategory {
    static func fromTMDBGenres(
        isMovie: Bool,
        genres: [TMDBGenre],
        overview: String?,
        originalLanguage: String? = nil,
        originCountries: [String]? = nil,
        networkNames: [String]? = nil
    ) -> String {
        let overviewLower = overview?.lowercased() ?? ""
        let lang = originalLanguage?.lowercased() ?? ""
        let countryUpper = Set((originCountries ?? []).map { $0.uppercased() })
        let networksLower = (networkNames ?? []).map { $0.lowercased() }

        func hasAnimationGenre() -> Bool {
            genres.contains { genre in
                genre.id == 16
                    || genre.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "animation"
            }
        }

        if genres.contains(where: { $0.name.lowercased().contains("anime") }) {
            return "anime"
        }

        let animeNetworkHints = ["crunchyroll", "funimation", "hidive", "muse", "aniplex", "viz media", "shonen jump", "mappa", "toei animation", "tv tokyo", "fuji tv", "nhk"]
        if !isMovie,
           networksLower.contains(where: { net in
               animeNetworkHints.contains { hint in net.contains(hint) }
           }) {
            return "anime"
        }

        let jpAnimation = (lang == "ja" || countryUpper.contains("JP")) && hasAnimationGenre()
        if jpAnimation { return "anime" }

        if hasAnimationGenre(), overviewLower.contains("japan") {
            return "anime"
        }

        if hasAnimationGenre() { return isMovie ? "movie" : "tv" }
...
class TMDBService: ObservableObject {
    ...
    private func requireHTTP200(_ response: URLResponse, data: Data) throws {
        guard let http = response as? HTTPURLResponse else {
            throw TMDBError.invalidResponse
        }
        guard http.statusCode == 200 else {
            throw TMDBError.httpStatus(http.statusCode)
        }
    ...
    func fetchMovieDetails(id: Int) async throws -> TMDBMovieDetails {
        ...
        let (data, response) = try await URLSession.shared.data(from: url)
        try requireHTTP200(response, data: data)
    ...
    func fetchFullDetails(id: Int, isMovie: Bool) async throws -> TMDBFullDetails {
        ...
        if isMovie {
            ...
            let netNames = (details.networks ?? []).map(\.name)
            return TMDBFullDetails(
